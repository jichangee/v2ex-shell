  ...
  try {
    ...
    $('.cell.item').each((i, element) => {
      ...
      const topic = {
        ...
        url: (BASE_URL + $element.find('.topic-link').attr('href')).split('#')[0],
        ...
        lastReplyAuthor: $element.find('.topic_info strong a').last().text().trim(),
  ...
  try {
    ...
    $('.cell[id^="r_"]').each((i, element) => {
      ...
      const reply = {
        ...
        content: $element.find('.reply_content').html().replace(/<br\s*\/?>/g, '\n').replace(/<[^>]*>/g, '').trim(),
    ...
    const prev = hasPagin ? !$('.ps_container > table table td:first-child').hasClass('disable_now') : false
    const next = hasPagin ? !$('.ps_container > table table td:last-child').hasClass('disable_now') : false
